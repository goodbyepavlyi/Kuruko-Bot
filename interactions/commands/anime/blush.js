...
const { BLUSH } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `blush`,
    description: `Posts a blush gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `blush`);

		return sendMessage(source, { embeds: [BLUSH(user.username, text, image)] });
	}
};
...
const { KISS } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `kiss`,
    description: `Posts a kiss gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `kiss`);

		return sendMessage(source, { embeds: [KISS(user.username, text, image)] });
	},
};
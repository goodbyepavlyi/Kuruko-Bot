...
const { SMILE } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `smile`,
    description: `Posts a smile gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `smile`);

		return sendMessage(source, { embeds: [SMILE(user.username, text, image)] });
	},
};
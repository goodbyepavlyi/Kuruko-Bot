...
const { CUDDLE } = require(`../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `cuddle`,
    description: `Posts a cuddle gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `cuddle`);

		return sendMessage(source, { embeds: [CUDDLE(user.username, text, image)] });
	}
};
...
const { FIGHT } = require(`../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `fight`,
    description: `Posts a fight gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `fight`);

		return sendMessage(source, { embeds: [FIGHT(user.username, text, image)] });
	}
};
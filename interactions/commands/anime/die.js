...
const { DIE } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `die`,
    description: `Posts a die gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `die`);

		return sendMessage(source, { embeds: [DIE(user.username, text, image)] });
	},
};
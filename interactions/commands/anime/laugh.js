...
const { LAUGH } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `laugh`,
    description: `Posts a laugh gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `laugh`);

		return sendMessage(source, { embeds: [LAUGH(user.username, text, image)] });
	},
};
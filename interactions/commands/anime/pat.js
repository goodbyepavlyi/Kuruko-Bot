...
const { PAT } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `pat`,
    description: `Posts a pat gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `pat`);

		return sendMessage(source, { embeds: [PAT(user.username, text, image)] });
	},
};
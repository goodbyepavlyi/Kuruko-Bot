...
const { CONFUSED } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `confused`,
    description: `Posts a confused gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `confused`);

		return sendMessage(source, { embeds: [CONFUSED(user.username, text, image)] });
	}
};
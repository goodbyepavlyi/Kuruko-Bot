...
const { TICKLE } = require(`../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `tickle`,
    description: `Posts a tickle gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `tickle`);

		return sendMessage(source, { embeds: [TICKLE(user.username, text, image)] });
	},
};
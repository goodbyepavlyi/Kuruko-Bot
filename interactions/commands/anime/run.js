...
const { RUN } = require(`../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `run`,
    description: `Posts a run gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `run`);

		return sendMessage(source, { embeds: [RUN(user.username, text, image)] });
	},
};
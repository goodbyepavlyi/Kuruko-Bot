...
const { BITE } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `bite`,
    description: `Posts a bite gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `bite`);

		return sendMessage(source, { embeds: [BITE(user.username, text, image)] });
	}
};
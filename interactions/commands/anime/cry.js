...
const { CRY } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `cry`,
    description: `Posts a cry gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `cry`);

		return sendMessage(source, { embeds: [CRY(user.username, text, image)] });
	}
};
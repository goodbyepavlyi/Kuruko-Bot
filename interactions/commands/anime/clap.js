...
const { CLAP } = require(`../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `clap`,
    description: `Posts a clap gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `clap`);

		return sendMessage(source, { embeds: [CLAP(user.username, text, image)] });
	}
};
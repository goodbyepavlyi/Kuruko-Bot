...
const { HIDE } = require(`../../../assets/messages.js`);

module.exports = {
    type: `command`,
    name: `hide`,
    description: `Posts a hide gif.`,
    options: [
        {
            type: 3,
            name: `text`,
	...
	async run(source, user, text) {
        const image = await kawaiiapi(`gif`, `hide`);

		return sendMessage(source, { embeds: [HIDE(user.username, text, image)] });
	},
};